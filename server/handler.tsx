...
import { handleBookingApi, type BookingEnvironment } from './booking-api';

const escape = (value: string) => value.replace(/[&<>"']/g, char => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[char]!);

function renderHead(metadata: Metadata, status: number, noIndex = false) {
  ...
  return '<title>' + escape(metadata.title) + '</title>'
    ...
    + (status === 404 || noIndex ? meta('robots', 'noindex, nofollow') : '')
...
export async function handleRequest(request: Request, template: string, env: BookingEnvironment = {}): Promise<Response | null> {
  const { pathname } = new URL(request.url);
  const bookingResponse = await handleBookingApi(request, env);
  ...
  const html = template.replace('<!--app-head-->', () => renderHead(route.metadata, route.status, route.noIndex))
